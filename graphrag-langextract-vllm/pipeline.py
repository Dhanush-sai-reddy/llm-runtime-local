...
OUTPUT_DIR = config.get("storage", {}).get("base_dir", "output")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Resolve provider-specific settings
if PROVIDER == "openrouter":
    provider_cfg = config["openrouter"]
    MODEL_NAME = provider_cfg["model"]
    MODEL_URL = provider_cfg["api_base"]
    API_KEY = os.environ.get("OPEN_ROUTER_KEY", "")
elif PROVIDER == "ollama":
    provider_cfg = config["ollama"]
    MODEL_NAME = provider_cfg["model"]
    MODEL_URL = provider_cfg["api_base"]
    API_KEY = "ollama"  # Ollama doesn't need a real key
else:
    raise ValueError(f"Unknown provider: {PROVIDER}. Use 'openrouter' or 'ollama'.")
